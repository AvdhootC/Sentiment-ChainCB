...
async function performAdvancedAnalysis(text) {
  await new Promise(resolve => setTimeout(resolve, 1000 + Math.random()*800)); // Longer delay
  const basicResults = await performBasicAnalysis(text); // Get base analysis

  // Simulate refinement (higher confidence, adjust percentages slightly towards dominant)
  let confidence = Math.min(99, basicResults.confidence * 1.1 + 8); // Increase confidence more
  let posPerc = basicResults.positive_percentage;
  let negPerc = basicResults.negative_percentage;
  let neuPerc = basicResults.neutral_percentage;

  // Slightly amplify the dominant sentiment
  if (basicResults.sentiment === 'Positive') {
      posPerc = Math.min(100, posPerc + 5);
      negPerc = Math.max(0, negPerc - 2);
  } else if (basicResults.sentiment === 'Negative') {
      negPerc = Math.min(100, negPerc + 5);
      posPerc = Math.max(0, posPerc - 2);
  }
   // Recalculate neutral
   neuPerc = Math.max(0, 100 - posPerc - negPerc);

  // Re-normalize
  const total = posPerc + negPerc + neuPerc;
   if (total > 0) {
      posPerc = (posPerc / total) * 100;
      negPerc = (negPerc / total) * 100;
      neuPerc = (neuPerc / total) * 100;
  }

  return {
    ...basicResults, // Keep original sentiment label and topics
    confidence: parseFloat(confidence.toFixed(1)),
    positive_percentage: parseFloat(posPerc.toFixed(1)),
    negative_percentage: parseFloat(negPerc.toFixed(1)),
    neutral_percentage: parseFloat(neuPerc.toFixed(1)),
    model: 'advanced' // Indicate the model used
  };
}
  ...
  try {
    ...
    const nonDemoMarketplaceItems = window.marketplaceItems.filter(item => !item.id.startsWith('demo-'));
     ...
     if (nonDemoMarketplaceItems.length > MAX_MARKETPLACE_STORAGE) {
         ...
         nonDemoMarketplaceItems = nonDemoMarketplaceItems.slice(0, MAX_MARKETPLACE_STORAGE);
     }
